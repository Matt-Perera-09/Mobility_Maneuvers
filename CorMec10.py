...
while play_again:
    ...
    # Button belongs near the bottom of the screen (Y = 500)
    replay_button = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 180, 200, 50)
    ...
    while end_running:
        ...
        # --- Endgame Text --- #
        # 1. Completion Praise
        congrats_text = sys_font.render(f"Congratulations {player_name}, you made it!", True, (255, 215, 0))
        ...
        if level_complete:
            ...
            if len(scores) == 0:
                ...
                screen.blit(empty_text, (SCREEN_WIDTH // 2 - empty_text.get_width() // 2, 270))
            

            # title_text = sys_font.render("#-# Top 5 Fastest Times #-#", True, (200, 200, 200))
            # screen.blit(title_text, (SCREEN_WIDTH // 2 - title_text.get_width() // 2, SCREEN_HEIGHT // 2 - 20))

            # Loop through the list of top 5 scores
            else:
                for i, entry in enumerate(scores[:5]):
                    ...
                    # Space each place out vertically by 30 pixels
                    screen.blit(score_surf, (SCREEN_WIDTH // 2 - score_surf.get_width() // 2, SCREEN_HEIGHT // 2 + 2- + (i * 30)))
    
        # 3. Draw the replay button
        pygame.draw.ellipse(screen, button_color, replay_button)
